...
function initIO(dispatch, userid) {
  ...
  if (!io.socket) {
    // 连接服务器, 得到与服务器的连接对象
    io.socket = io("http://13.239.140.145"); // 2. 创建对象之后: 保存对象
...
export const register = (user) => {
  ...
  if (!username) {
    Toast.info("username is required!", 2);
    return errorMsg("用户名必须指定!");
  } else if (password.length < 4) {
    Toast.info("password must be more than 4 letters!", 2);
    return errorMsg("密码不能少于4位!");
  } else if (password !== password2) {
    Toast.info("confirm password must be same!", 2);
  ...
  return async (dispatch) => {
    ...
    if (result.code === 0) {
      ...
      // 失败
      // 分发错误提示信息的同步action
      Toast.info("user exists already!", 2);
...
export const login = (user) => {
  ...
  if (!username) {
    ...
    Toast.info("password is required!", 2);
  ...
  return async (dispatch) => {
    ...
    if (result.code === 0) {
      ...
      // 失败
      // 分发错误提示信息的同步action
      Toast.info("username or password incorrect!", 2);
      dispatch(errorMsg(result.msg));
    }
  };
};
